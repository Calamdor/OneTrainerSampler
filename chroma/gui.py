...
CONFIG_PATH = os.path.join(os.path.dirname(_SCRIPT_DIR), "chroma_sampler_config.json")
...
class ChromaSamplerApp(BaseSamplerApp):
    ...
    def _build_model_frame(self, pad: dict) -> None:
        ...
        _dtype_cb = ttk.Combobox(
            ...
            values=TRANSFORMER_DTYPE_OPTIONS, state="readonly", width=8,
        )
        ...
        Tooltip(_dtype_cb,
                ...
                "GGUF         — loads a separate .gguf file; enables GGUF path field.\n\n"
                "torch.compile only benefits BF16/FP16 — quantized dtypes use\n"
                "custom CUDA kernels that compile cannot optimize.")
        ...
        Tooltip(_gguf_entry,
                ...
                "Visible only when Transformer dtype is set to GGUF.\n"
        ...
        btn_row.grid(row=r, column=0, columnspan=3, sticky="w", **pad)
        _load_btn = ttk.Button(btn_row, text="Load Model",  command=self._load_model)
        _load_btn.pack(side="left", padx=2)
        Tooltip(_load_btn,
                "Load (or reload) the model with the current settings.\n\n"
                "Required before generating. If settings change after loading,\n"
                "a warning is shown — click Load again to apply the new settings.")
        _unload_btn = ttk.Button(btn_row, text="Unload",      command=self._unload_model)
        _unload_btn.pack(side="left", padx=2)
        Tooltip(_unload_btn, "Unload the model and free all GPU/CPU memory.\n"
                "Use this before loading a different model or dtype.")
        self._model_status_var = tk.StringVar(value="Not loaded")
        self._model_status_label = tk.Label(
            btn_row, textvariable=self._model_status_var, foreground="gray",
            background=BG,
        )
        self._model_status_label.pack(side="left", padx=8)

        model_frame.columnconfigure(1, weight=1)

        # Traces — warn when model-affecting settings change after load
        for _var in (self._base_model_var, self._dtype_var, self._te_dtype_var,
                     self._svd_var, self._svd_rank_var, self._svd_dtype_var,
                     self._cache_var, self._compile_var,
                     self._offload_enabled_var, self._offload_fraction_var,
                     self._gguf_var, self._attn_var,
                     self._compute_dtype_var, self._fast_fp16_var):
            _var.trace_add("write", self._on_model_setting_changed)
    ...
    def _load_model(self):
        if self._busy:
            return
        base = self._base_model_var.get().strip()
        if not base:
            messagebox.showwarning("No model", "Enter the base model path first.")
            return
        self._set_busy(True)
        self._model_status_var.set("Loading …")
        cfg = self._collect_cfg()

        import threading
        def _work():
            try:
                self.backend.load_model(cfg, on_status=lambda s: self.root.after(
                    0, self._set_model_status, s))
            except Exception as exc:
                self.root.after(0, self._set_model_status, f"Error: {exc}")
            finally:
    ...
    def _unload_model(self):
        if self._busy:
            return
        self._run_in_thread(
            lambda: self.backend.unload_model(
                on_status=lambda s: self.root.after(0, self._set_model_status, s),
            )
        )
